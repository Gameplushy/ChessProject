...
public class ChessBoard implements Serializable {
    ...
    public void checkPossibleMoves(int abs, int ord){
        Piece p = board[abs][ord];
        int[] coord = new int[]{abs,ord};
        if(p==null||p.isWhite()!=isTurnForWhite){ System.out.println("Not a piece!"); return;}
    ...
    */
    private int isMovePossible(boolean isAColor,int[] coord){
        if(coord[0]<0||coord[0]>8||coord[1]<0||coord[1]>7) return 3;
